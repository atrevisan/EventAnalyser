...
class TwitterDataAnalysis:
    ...
    def get_top_hashtags(self, max_hashtags=20):
        """Get the top hashtags (most frequent) extracted from some dataset.
    
        Parameters
        ----------        
        max_hashtags : int
            Max number of top hashtags to be retrived. 
            
        Returns
        -------
        top_hashtags : list of tuples (string, number)
        ...
        for tweet in self.tweets:
            ...
            hashtags_list = re.findall(r'#([^\s]+)', tweet[3])
        ...
        top_hashtags = sorted(list(top_hashtags.items()), key=lambda x : -x[1])

        if len(top_hashtags) >= max_hashtags:

            return top_hashtags[:max_hashtags]
        else:
            return top_hashtags

    def generate_hashtag_frequency_info_per_month(self, hashtag, year):
        """Generate a mapping for each month of the year to its info.
        
        The infos for a given month regarding a given hashtag are: the hashtag absolute frequency
        in that month, the mean frequency in that month and the max frequency
        in that month. The data generated by this function will be used for the plot of the 
        distribution.
        ...
        -----------
        hashtag : str
            The hashtag for which the distribution will be generated.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max frequency of the hashtag in the month.
        """

        # store the info of the given hashtag per month
        hashtag_info_per_month = {}
        for tweet in self.tweets:

            tweet_time = tweet[1]
            tweet_text = tweet[3]
            
            # Do The hashtag belongs to the tweet?
            if hashtag in tweet_text:
            
                month = tweet_time.split()[1]
                day = tweet_time.split()[2]

                if month in hashtag_info_per_month:

                    month_info = hashtag_info_per_month[month]
                    hashtag_month_frequency = month_info[0]
                    hashtag_daily_frequency = month_info[1]

                    hashtag_month_frequency += tweet_text.count(hashtag)
                    
                    if day in hashtag_daily_frequency:
                        
                        hashtag_daily_frequency[day] +=  tweet_text.count(hashtag)

                    else:

                        hashtag_daily_frequency[day] = tweet_text.count(hashtag)

                    month_info = (hashtag_month_frequency, hashtag_daily_frequency)
                    hashtag_info_per_month[month] = month_info

                else:

                    hashtag_month_frequency = tweet_text.count(hashtag)
                    hashtag_daily_frequency = {}
                    hashtag_daily_frequency[day] = tweet_text.count(hashtag)
                    
                    month_info = (hashtag_month_frequency, hashtag_daily_frequency)
                    hashtag_info_per_month[month] = month_info

        # calculate average and max frequency of the hashtag in the month
        plot_info = []
        for month_item in hashtag_info_per_month.items():

            month_info = month_item[1]
            hashtag_month_frequency = month_info[0]
            hashtag_daily_frequency = month_info[1]

            hashtag_average_frequency = hashtag_month_frequency / len(hashtag_daily_frequency)
            hashtag_max_frequency = hashtag_daily_frequency[max(hashtag_daily_frequency, key=lambda k: hashtag_daily_frequency[k])]

            plot_info.append((month_item[0], hashtag_month_frequency, hashtag_average_frequency, hashtag_max_frequency))

        return plot_info

    def generate_hashtag_frequency_info_per_day(self, hashtag, month, year):
        """Generate a mapping for each day of the month to its info.
        
        The infos for a given day regarding a given hashtag are: the hashtag absolute frequency
        in that day, the mean frequency in that day and the max frequency
        in that day. The data generated by this function will be used for the plot of the 
        distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max frequency of the hashtag in the day.
        """

        # store the info of the given hashtag per day
        hashtag_info_per_day = {}
        for tweet in self.tweets:
            ...
            # Do The hashtag belongs to the tweet?
            if hashtag in tweet_text and month == tweet_month and year == tweet_year:
             
                day = tweet_time.split()[2]
                hour = tweet_time.split()[3].split(":")[0]

                if day in hashtag_info_per_day:

                    day_info = hashtag_info_per_day[day]

                    hashtag_day_frequency = day_info[0]
                    hashtag_hourly_frequency = day_info[1]

                    hashtag_day_frequency += tweet_text.count(hashtag)
                    
                    if hour in hashtag_hourly_frequency:
                        
                        hashtag_hourly_frequency[hour] +=  tweet_text.count(hashtag)

                    else:

                        hashtag_hourly_frequency[hour] = tweet_text.count(hashtag)

                    day_info = (hashtag_day_frequency, hashtag_hourly_frequency)
                    hashtag_info_per_day[day] = day_info

                else:

                    hashtag_day_frequency = tweet_text.count(hashtag)
                    hashtag_hourly_frequency = {}
                    hashtag_hourly_frequency[hour] = tweet_text.count(hashtag)
                    
                    day_info = (hashtag_day_frequency, hashtag_hourly_frequency)
                    hashtag_info_per_day[day] = day_info

        # calculate average and max frequency of the hashtag in the day
        plot_info = []
        for day_item in hashtag_info_per_day.items():

            day_info = day_item[1]
            hashtag_day_frequency = day_info[0]
            hashtag_hourly_frequency = day_info[1]

            hashtag_average_frequency = hashtag_day_frequency / len(hashtag_hourly_frequency)
            hashtag_max_frequency = hashtag_hourly_frequency[max(hashtag_hourly_frequency, key=lambda k: hashtag_hourly_frequency[k])]

            plot_info.append((day_item[0], hashtag_day_frequency, hashtag_average_frequency, hashtag_max_frequency))

        return plot_info

    def generate_hashtag_frequency_info_per_hour(self, hashtag, day, month, year):
        """Generate a mapping for each hour of the day to its info.
        
        The infos for a given hour regarding a given hashtag are: the hashtag absolute frequency
        in that hour, the mean frequency in that hour and the max frequency
        in that hour. The data generated by this function will be used for the plot of the 
        distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max frequency of the hashtag in the hour.
        """

        # store the info of the given hashtag per hour
        hashtag_info_per_hour = {}
        for tweet in self.tweets:
            ...
            # Do The hashtag belongs to the tweet?
            if hashtag in tweet_text and month == tweet_month and year == tweet_year and day == tweet_day:
             
                hour = tweet_time.split()[3].split(":")[0]
                minute = tweet_time.split()[3].split(":")[1]

                if hour in hashtag_info_per_hour:

                    hour_info = hashtag_info_per_hour[hour]

                    hashtag_hour_frequency = hour_info[0]
                    hashtag_minutely_frequency = hour_info[1]

                    hashtag_hour_frequency += tweet_text.count(hashtag)
                    
                    if minute in hashtag_minutely_frequency:
                        
                        hashtag_minutely_frequency[minute] +=  tweet_text.count(hashtag)

                    else:

                        hashtag_minutely_frequency[minute] = tweet_text.count(hashtag)

                    hour_info = (hashtag_hour_frequency, hashtag_minutely_frequency)
                    hashtag_info_per_hour[hour] = hour_info

                else:

                    hashtag_hour_frequency = tweet_text.count(hashtag)
                    hashtag_minutely_frequency = {}
                    hashtag_minutely_frequency[minute] = tweet_text.count(hashtag)
                    
                    hour_info = (hashtag_hour_frequency, hashtag_minutely_frequency)
                    hashtag_info_per_hour[hour] = hour_info

        # calculate average and max frequency of the hashtag in the hour
        plot_info = []
        for hour_item in hashtag_info_per_hour.items():

            hour_info = hour_item[1]
            hashtag_hour_frequency = hour_info[0]
            hashtag_minutely_frequency = hour_info[1]

            hashtag_average_frequency = hashtag_hour_frequency / len(hashtag_minutely_frequency)
            hashtag_max_frequency = hashtag_minutely_frequency[max(hashtag_minutely_frequency, key=lambda k: hashtag_minutely_frequency[k])]

            plot_info.append((hour_item[0], hashtag_hour_frequency, hashtag_average_frequency, hashtag_max_frequency))

        return plot_info

    def generate_hashtag_retweet_info_per_month(self, hashtag, year):
        """Generate a mapping for each month of the year to its info.
        
        The infos for a given month regarding the number of retweets for the hashtag are: 
        the retweets absolute frequency in that month, the retweets mean frequency in that 
        month and the retweets max frequency in that month. The data generated by this function 
        will be used for the plot of the distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max frequency of retweets to the hashtag in the month.
        """
        ...
        for tweet in self.tweets:
            ...
            if hashtag in tweet_text:
                ...
                if month in hashtag_info_per_month:
                    ...
                    hashtag_month_frequency += tweet_number_of_retweets
                    
                    if day in hashtag_daily_frequency:
                        
                        hashtag_daily_frequency[day] +=  tweet_number_of_retweets
        ...
        # calculate average and max retweets of the hashtag in the month
        plot_info = []
        for month_item in hashtag_info_per_month.items():
    ...
    def generate_hashtag_retweet__info_per_day(self, hashtag, month, year):
        """Generate a mapping for each day of the month to its info.
        
        The infos for a given day regarding the number of retweets for a given hashtag are:
        the retweets absolute frequency in that day, the retweets mean frequency in that day 
        and the retweets max frequency in that day. The data generated by this function will 
        be used for the plot of the distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max frequency of retweets for the hashtag in the day.
        """
        ...
        for tweet in self.tweets:
            ...
            if hashtag in tweet_text and month == tweet_month and year == tweet_year:
                ...
                if day in hashtag_info_per_day:
                    ...
                    hashtag_day_frequency += tweet_number_of_retweets
                    
                    if hour in hashtag_hourly_frequency:
                        
                        hashtag_hourly_frequency[hour] +=  tweet_number_of_retweets
        ...
        # calculate average and max retweets of the hashtag in the day
        plot_info = []
        for day_item in hashtag_info_per_day.items():
    ...
    def generate_hashtag_retweet_info_per_hour(self, hashtag, day, month, year):
        """Generate a mapping for each hour of the day to its info.
        
        The infos for a given hour regarding the number of retweets for a given hashtag are: 
        the retweets absolute frequency in that hour, the retweets mean frequency in that hour 
        and the retweets max frequency in that hour. The data generated by this function will be 
        used for the plot of the distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max  retweets of the hashtag in the hour.
        """
        ...
        for tweet in self.tweets:
            ...
            if hashtag in tweet_text and month == tweet_month and year == tweet_year and day == tweet_day:
                ...
                if hour in hashtag_info_per_hour:
                    ...
                    hashtag_hour_frequency += tweet_number_of_retweets
                    
                    if minute in hashtag_minutely_frequency:
                        
                        hashtag_minutely_frequency[minute] +=  tweet_number_of_retweets
        ...
        # calculate average and max retweets for the hashtag in the hour
        plot_info = []
        for hour_item in hashtag_info_per_hour.items():
    ...
    def generate_hashtag_sentiment_info_per_month(self, hashtag, year, classifier, feature_extractor, sentiment="positive"):
        """Generate a mapping for each month of the year to its info.
        
        The infos for a given month regarding the number of tweets with the intended sentiment for the hashtag are: 
        the intended sentiment tweets absolute frequency in that month, the intended sentiment mean frequency in that 
        month and the intended sentiment max frequency in that month. The data generated by this function 
        will be used for the plot of the distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max frequency of tweets with the intended sentiment related to the hashtag in the month.
        """
        ...
        for tweet in self.tweets:
            ...
            # Do The hashtag belongs to the tweet?
            if hashtag in tweet_text and intended_sentiment == predicted_class:
                ...
                if month in hashtag_info_per_month:
                    ...
                    hashtag_month_frequency += 1
                    
                    if day in hashtag_daily_frequency:
                        
                        hashtag_daily_frequency[day] +=  1
        ...
        # calculate average and max intended sentiment for the hashtag in the month
        plot_info = []
        for month_item in hashtag_info_per_month.items():
    ...
    def generate_hashtag_sentiment_info_per_day(self, hashtag, month, year, classifier, feature_extractor, sentiment="positive"):
        """Generate a mapping for each day of the month to its info.
        
        The infos for a given day regarding the number of tweets with the intended sentiment for a given hashtag are:
        the intended sentiment absolute frequency in that day, the  mean frequency in that day 
        and the max frequency in that day. The data generated by this function will 
        be used for the plot of the distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max frequency of tweets with the intended sentiment for the hashtag in the day.
        """
        ...
        for tweet in self.tweets:
            ...
            # Do The hashtag belongs to the tweet?
            if hashtag in tweet_text and month == tweet_month and year == tweet_year and intended_sentiment == predicted_class:
                ...
                if day in hashtag_info_per_day:
                    ...
                    hashtag_day_frequency += 1
                    
                    if hour in hashtag_hourly_frequency:
                        
                        hashtag_hourly_frequency[hour] +=  1
        ...
        # calculate average and max tweets with the intended sentiment for the hashtag in the day
        plot_info = []
        for day_item in hashtag_info_per_day.items():
    ...
    def generate_hashtag_sentiment_info_per_hour(self, hashtag, day, month, year, classifier, feature_extractor, sentiment="positive"):
        """Generate a mapping for each hour of the day to its info.
        
        The infos for a given hour regarding the number of tweets with the intended sentiment for a given hashtag are: 
        the absolute frequency in that hour, the mean frequency in that hour 
        and the max frequency in that hour. The data generated by this function will be 
        used for the plot of the distribution.
        ...
        --------
        plot_info : list of tuples (str, int, float, int)
            The total, average and max tweets with the intended sentiment for the hashtag in the hour.
        """
        ...
        for tweet in self.tweets:
            ...
            # Do The hashtag belongs to the tweet?
            if hashtag in tweet_text and month == tweet_month and year == tweet_year and day == tweet_day and intended_sentiment == predicted_class:
                ...
                if hour in hashtag_info_per_hour:
                    ...
                    hashtag_hour_frequency += 1
                    
                    if minute in hashtag_minutely_frequency:
                        
                        hashtag_minutely_frequency[minute] +=  1
        ...
        # calculate average and max tweets with the intended sentiment for the hashtag in the hour
        plot_info = []
        for hour_item in hashtag_info_per_hour.items():
